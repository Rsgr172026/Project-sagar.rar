...
def createImage(flag=0):
    ...
    image_data = image_generated.read()
    image_display = ImageTk.PhotoImage(Image.open(io.BytesIO(image_data)))
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Error loading reload image: {e}")
        reload_img = None

    if reload_img:
        reload_button = Button(image=reload_img, command=lambda: createImage(1))
    else:
        reload_button = Button(text="Reload", command=lambda: createImage(1))
